...
/* ===================== 自托管：SQLite 适配器 =====================
 * 用 better-sqlite3 提供与 Cloudflare D1 兼容的接口，
 * 让本文件其余所有查询函数无需改动即可在 Node 服务器上运行。
 * D1 的 ?N 编号占位符在 better-sqlite3 中改写为 ?（保持顺序）。
 */
...
export interface DBLike {
  ...
  /** 对标 Cloudflare D1 的 batch：一次性执行多句已绑定的语句（事务内原子执行） */
  batch(
    stmts: D1Stmt[]
...
let _raw: import("better-sqlite3").Database | null = null;
...
function openDB() {
  ...
  const wrap = (
    ...
    // each D1Stmt captures its own bound values.
    // D1 uses ?N numbered placeholders; better-sqlite3 ONLY supports anonymous `?`
    // (it does NOT understand ?1/?2). So:
    //  - prepare() rewrites ?N -> ? (anonymous), recording each ?'s original index in `order`.
    //  - bind(...a) receives values for ?1,?2,... and we EXPAND them to fill every `?`
    //    occurrence in order (a repeated index => same value fills multiple positions,
    //    e.g. pruneRevisions' post_id=?1 used twice).
    const make = (expanded: unknown[]): D1Stmt => ({
      bind: (...a: unknown[]) => {
        const exp = order.length ? order.map((idx) => a[idx - 1]) : a;
        return make(exp);
      },
      all: <T = unknown>() => ({ results: stmt.all(...expanded) as T[] }),
      first: <T = unknown>() => (stmt.get(...expanded) ?? null) as T | null,
      run: () => {
        const r = stmt.run(...expanded);
        return { meta: { changes: r.changes, last_row_id: Number(r.lastInsertRowid) } };
      },
      raw: <T = unknown>() => stmt.raw().all(...expanded) as T[],
    });
    return make([]);
  };
  ...
  const db: DBLike = {
    prepare(sql: string) {
      // better-sqlite3 不支持 ?N 编号占位符，改写为匿名 ?；
      // 同时记录每个 ? 对应的原编号，bind 时按出现顺序展开（同一编号多次出现则同一值填多处）。
      const order = [...sql.matchAll(/\?(\d+)/g)].map((m) => Number(m[1]));
      const sqliteSql = sql.replace(/\?(\d+)/g, "?");
      return wrap(raw.prepare(sqliteSql), order);
    },
    exec(sql: string) {
      raw.exec(sql);
    },
    async batch(stmts: D1Stmt[]) {
      // better-sqlite3 无原生 batch；用事务包裹逐句执行，等价于 D1 batch 的原子多语句
      const txn = raw.transaction(() => stmts.map((s) => s.run()));
      return txn();
    },
  };
  return { db, raw };
...
/** 返回底层 better-sqlite3 实例（迁移 / 批量写入使用） */
