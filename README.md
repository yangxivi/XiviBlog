...
- **国家公祭日自动素灰**：命中九一八事变纪念日、南京大屠杀死难者国家公祭日等纪念日当天，整站自动切换为「纪念灰」并整体去色（连封面图一起变灰），跨过北京时间零点即生效；日期表可在后台自定义，也可一键关闭；临近时页脚会低调提示下一个公祭日（提前量可配，默认 7 天）。
...
**后台辅助**（都在「站点设置 → 国家公祭日」里）：

- **日期自检**：实时显示「今天 / 明天 / 后天」是否命中纪念日，改完日期表立刻能看到对不对。
- **下一个公祭日倒计时**：显示最近一个纪念日的日期、名称和「还有 N 天」，跨年自动顺延。
- **格式校验**：无法识别的条目会标红列出（例如把 `12-13` 写成 `12月13日`），避免手误被静默丢弃。
- **整站素灰预览**：一键临时给 `<html>` 打上公祭日标记，当场看整站变灰的效果，再点一次还原，不影响已保存设置。
- **页脚提醒提前量**：`0-60` 天（`memorialLeadDays`，默认 `7`），控制前台页脚那行小字从几天前开始出现；设为 `0` 即只在纪念日当天提醒。输入框下方会实时显示「按当前设置，页脚提醒正在显示 / 暂不显示（还差 N 天）」。

**前台提醒**：距离最近一个纪念日 ≤ `memorialLeadDays` 天时，页脚最底部会多出一行居中灰色小字（今天 / 明天 / N 天后），平时**完全不渲染**，页脚保持干净。实现见 `app/components/Footer.tsx`，触发时机读后台设置，时区口径与整站变灰完全一致（都走 `cnDay()` 东八区），避免「文案说还有 1 天、整站却已经变灰」。后台关闭「纪念日自动素灰」时同步不显示。
...
曦微出品，欢迎访问 https://blog.aixivi.cn/guestbook 交流讨论。
